...
""" - - Other parameters - - """
train_freq = 45 #best 20
epsilon = 0.05
ep_decay = 0.05 ** (1 / (0.85 * (NUM_EPISODES / train_freq) ) ) #reaches 5% at 75% training
global_step = 0
time_step = 0
soc_bat = 7.5
soc_ev = 15
EPOCH_MC = 6 #best 8
NUM_SAMPLES_MC = 150 #best 75
BUFFER_SIZE = 1500
...
df = df[
    ((df['utc_timestamp'] >= start_date) & (df['utc_timestamp'] < end_date))
]
""" - - Train with particular dates - - """
...
for m in [3, 5, 9]:
    ...
    start_date = pd.Timestamp(year=year, month=m, day=15)
    end_date = pd.Timestamp(year=year, month=m, day=18)  # exclusive upper bound
...
""" - - Start agent - - """
agent = DQNAgent(state_dim=state_dim, action_dim=action_dim, norm_factors=norm_factors, device=device, total_eps=NUM_EPISODES, buffer_size=BUFFER_SIZE)
...
def evaluate_year(num_days=NUM_DAYS):
    ...
    state, info = env.reset(time_step, soc_bat,soc_ev)
    done = False
    rewards = 0
...
def get_improvement_days():
    ...
    best_values = diff_values[int(len(diff_values) * 0.8):]
...
if is_training:
    ...
    t = time.time()
    rew = [0] * NUM_EPISODES
    ev_pen = [0] * NUM_EPISODES

    """ - - Run episodes - - """
    for ep in range(NUM_EPISODES):

        if ep % 3 == 0:
            soc_bat, soc_ev = 7.5, 15

        state, _ = env.reset(time_step, soc_bat, soc_ev)
        done = False
        ...
        while not done:
            ...
            time_step = info['time step'] + 1
            soc_bat = info['soc_bat']
            soc_ev = info['soc_ev']

            state = next_state

        if (ep % train_freq == 0) and (ep > BUFFER_SIZE):
        ...
        if (ep % p == 0) and ep != 0:
            ...
            print('Ep ', ep, 'EV Penalty: ', sum(ev_pen[(ep-p):ep]) / len(ev_pen[(ep-p):ep]))
    ...
    evaluate_year()
    evaluate_without_battery()

    soc_batt_array = states_array[:, 1]
    soc_ev_array = states_array[:, 2]
    price_buy_array = states_array[:, 4]
    price_sell_array = states_array[:, 4] * 0.8
    pv_array = states_array[:, 5]
    load_array = states_array[:, 6]
    # Time index
    ...
    l2, = ax1.plot(t, soc_ev_array, label='EV SOC')
